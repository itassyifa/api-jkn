                                        ...
                                        if(empty($booking['status'])) {
                                            ...
                                            if($booking['status']=='Batal'){
                                                ...
                                                if($interval>=60){
                                                    ...
                                                    $response = array(
                                                        'metadata' => array(
                                                            'message' => 'Checkin bisa dilakukan 1 jam sebelum dan sesudah Poli dibuka',
                                                            'code' => 201
                                                        )
                                                    );  
                                                    http_response_code(201);
                                           ...
                                           if($setrm["posisi_tahun_bulan"]=="Depan"){
                                                ...
                                                switch ($setrm["urutan"]) {
                                                    case "Straight":
                                                        ...
                                                        break;
                                                    case "Terminal":
                                                        $max    = getOne2("select ifnull(MAX(CONVERT(CONCAT(SUBSTRING(LEFT(no_rkm_medis,6),5,2),SUBSTRING(LEFT(no_rkm_medis,6),1,2)),signed)),0)+1 from set_no_rkm_medis");
                                                        $nourut = substr(sprintf("%06s", $max),4,2).substr(sprintf("%06s", $max),2,2).substr(sprintf("%06s", $max),0,2);
                                                        break;
                                                    case "Middle":
                                                        $max    = getOne2("select ifnull(MAX(CONVERT(CONCAT(SUBSTRING(LEFT(no_rkm_medis,6),3,2),SUBSTRING(LEFT(no_rkm_medis,6),5,2)),signed)),0)+1 from set_no_rkm_medis");
                                                        $nourut = substr(sprintf("%06s", $max),2,2).substr(sprintf("%06s", $max),0,2).substr(sprintf("%06s", $max),4,2);
                                                        break;
                                                }            
                                            }
